...
type
  ...
  public
    ...
    procedure RefreshImagesList(MineralName:String);
    { public declarations }
  end;
...
implementation
uses unitfichaespecie, unitframelist;
...
procedure TFrameImages.RefreshImagesList(MineralName: String);
var SQLstr:String;
begin
  {SQLstr:='SELECT imagem FROM '+Dados.Table5+' WHERE nome = "'+MineralName+'" ;';
//  Dados.TableImages:= Dados.DatabaseMinerals.GetTable();
  for I:=0 to Dados.TableImages.Count-1 do
  begin
    ...
    begin
      Parent := ScrollBoxImages;
      Left := ImagesLeft;
      Top := (I+1) * (ImagesMargin + ImagesSize);
      //Top := Image1Top + (ImagesNumber * SeparadorImagens);
      Width := ImagesWidth;
      Height := ImagesHeight;
      Proportional := True;
